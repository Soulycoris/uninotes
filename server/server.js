...
router.get('/get/unit_data/base/:unit_id', async (ctx, next) => {
  ...
  const uniqueEquipmentEnhance = await db.queryData(
    DBHelper.getUniqueEquipmentEnhance,
    {
      $unit_id: ctx.params.unit_id,
    }
  );
  ...
  const skillData = await db.queryData(
    `${DBHelper.getSkillData} ( ${skillArr.map(() => '?').join(',')} )`,
    skillArr
  );
  ...
  const skillAction = await db.queryData(
    `${DBHelper.getSkillAction} ( ${actionArr.map(() => '?').join(',')} )`,
    actionArr
  );
